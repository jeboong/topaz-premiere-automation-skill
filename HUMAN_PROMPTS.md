...
C:\path\clips
Output MOV files beside the originals or into an ups folder.
Keep the original base names.
If a source is already 24fps, skip frame interpolation.
Verify that every output is 3840x2160, exactly 24fps, and ProRes 4444.
Do not access X drive.
```
...
Use the company upscale preset: Rhea 4K, Apollo only for non-24fps sources, MOV ProRes 4444, audio copy, duplicate frames off, final CFR 24fps.
Do not access X drive.
```
...
- Output MOV ProRes 4444 per company rule.
...
- "company upscale preset" means Rhea 4K, Apollo only for non-24fps sources, final CFR 24fps, MOV ProRes 4444, audio copy, duplicate-frame replacement off.
